...
bool excluirElementoPilha(PILHATRIPLA* p, REGISTRO* reg, int pilha){
   ...
   if (pilha == 1) {
    if (p->topo1 == -1) return resposta; // verificar se a pilha 1 está vazia
    *reg = p->A[p->topo1];
    p->topo1 = p->topo1 - 1;
    resposta = true;
   }else if (pilha == 2){
    if (p->topo2 == (p->base2) - 1) return resposta; // verificar se a pilha 2 está vazia
    *reg = p->A[p->topo2];
    p->topo2 = p->topo2 - 1;
    resposta = true;
   }else{
    if (p->topo3 == MAX) return resposta; // verificar se a pilha 3 está vazia
    *reg = p->A[p->topo3];
    p->topo3 = p->topo3 + 1;
    resposta = true;
   }
